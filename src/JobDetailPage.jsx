...
import { ArrowLeft, CheckCircle2, Clock3, Download, FileImage, History, Loader2, Wrench } from 'lucide-react';
...
export default function JobDetailPage({ jobId, onBack }) {
  ...
  if(error)return <div className="error-box">{error}<button className="text-btn" onClick={load}>Retry</button></div>;
  if(!job)return null;
  ...
  return <div className="detail-page">
    ...
    {job.source_document_path&&<section className="detail-card"><h2><FileImage size={19}/> Original jobsheet</h2><p className="muted">The original source remains attached as evidence.</p><button className="secondary" onClick={async()=>{const u=await getPrivateFileUrl(job.source_document_path);window.open(u,'_blank','noopener,noreferrer');}}><Download size={17}/> Open original</button></section>}
