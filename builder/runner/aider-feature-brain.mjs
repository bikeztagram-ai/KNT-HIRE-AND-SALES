#!/usr/bin/env node
/**
 * KNT Hire & Sales AutoBot — Aider-backed feature engineer.
 * Aider owns repository-aware edit/test execution; KNT owns objective,
 ...
 * and the same KNT objective resumes on the next run.
 */
...
const productMarker='KNT-HIRE-AND-SALES';
...
function loadObjectives(){
  ...
  try{return JSON.parse(fs.readFileSync(file,'utf8')).objectives||[];}
    ...
    try{
      ...
      if(changed){
        console.error('[aider] protected KNT feature-objectives.json was modified/corrupted; restoring tracked version.');
    ...
    throw new Error(`KNT feature objectives JSON is invalid and could not be safely restored: ${error.message}`);
  }
}
...
if(loadedState.recovered){
  console.warn(`[aider] recovered persistent KNT state from ${loadedState.source}; continuing safely instead of aborting.`);
}
...
function promptFor(obj,pass){
  ...
  return [
    ...
    `Objective-scoped KNT product files — use these exact repository paths: ${files.join(', ')}`,
    `Objective constraints: ${JSON.stringify(obj.constraints||[])}`,
    'Inspect the supplied KNT product files and their relevant callers/contracts as needed. Make one coherent, real production improvement for KNT Hire & Sales for this objective.',
    ...
    'Never rename, substitute, guess or invent a KNT filename. The exact paths supplied above are authoritative for this pass.',
    'Preserve public contracts and all existing KNT safety, scope, rollback, audit, production verification, Supabase, evidence, authentication and human-invoice-review rules.',
    'Run the narrowest relevant verification. Do not merely describe changes: actually edit the supplied KNT files.',
...
function restorePassSnapshot(obj,snapshot){
  ...
  try{if(fs.existsSync(snapshot)&&fs.statSync(snapshot).size>0)execFileSync('git',['apply','--whitespace=nowarn',snapshot],{cwd:root,stdio:'inherit'});}catch(error){console.error(`[aider] failed to restore KNT pass snapshot: ${error.message}`);}
...
function assertScope(before,obj){
  ...
  if(unauthorized.length){
    console.error(`[aider] unauthorized KNT modified paths: ${unauthorized.join(', ')}`);
    for(const file of unauthorized){
      try{execFileSync('git',['restore','--',file],{cwd:root,stdio:'inherit'});}catch{}
      try{execFileSync('git',['clean','-fd','--',file],{cwd:root,stdio:'inherit'});}catch{}
    }
    throw new Error(`Aider modified files outside KNT objective scope: ${unauthorized.join(', ')}`);
  }
}
function verifyDiff(){execFileSync('git',['diff','--check'],{cwd:root,stdio:'inherit'});}
function verifyBuild(){
  const remaining=remainingMs();
  if(remaining<35_000)throw new Error('insufficient remaining run budget for KNT build verification');
...
const obj=objective();
if(!obj){console.log(JSON.stringify({ok:true,protocol,status:'no-eligible-objective',product:productMarker}));process.exit(0);}
const files=scopedFiles(obj);
if(!files.length){console.error(`[aider] KNT objective ${obj.id} has no scoped files`);process.exit(1);}
for(const file of files){if(!fs.existsSync(path.join(root,file))){console.error(`[aider] KNT objective ${obj.id} references missing exact file: ${file}`);process.exit(1);}}
...
for(let pass=firstPass;pass<=maxPasses;pass++){
  const remaining=remainingMs();
  if(remaining<35_000){console.error('[aider] KNT hard feature deadline reached before next pass; continuation checkpoint remains durable.');break;}
  if(normalRemainingMs()<35_000&&pass>firstPass){console.error('[aider] KNT normal work deadline reached; no new pass will start.');break;}
  ...
  console.log(`[aider] KNT ${obj.id} pass ${pass}/${maxPasses}; exact files=${aiderFiles.join(',')}; cwd=${aiderCwd}`);
  const result=spawnSync('aider',args,{cwd:aiderCwd,encoding:'utf8',stdio:'inherit',timeout});
  if(result.error){
    console.error(`[aider] KNT pass ${pass} stopped: ${result.error.code||result.error.message}; rolling back only this pass`);
  ...
  if(result.status!==0){
    console.error(`[aider] KNT pass ${pass} exited ${result.status}; rolling back only this pass`);
    restorePassSnapshot(obj,snapshot);
  ...
  try{
    ...
    console.error(`[aider] KNT pass ${pass} verification failed: ${error.message}; rolling back only this pass`);
    restorePassSnapshot(obj,snapshot);
...
saveAiderState(statePath,state);
console.log(JSON.stringify({ok:success,protocol,product:productMarker,objective:obj.id,passes:maxPasses,startingPass:firstPass,model,exactFiles:files,elapsedMs:(requestedMinutes*60_000)-remainingMs(),remainingMs:remainingMs(),normalRemainingMs:normalRemainingMs(),resumable:!success&&state.inProgress?.id===obj.id,stateRecovery:loadedState.recovered?loadedState.source:null}));
