#!/usr/bin/env node
/**
 * KNT sustained AutoBot controller.
 * Mirrors the proven long-run feature loop: shared deadline, small repository-
 * aware feature passes, durable state, audit integrity and no-progress stop.
 */
...
const featureStatePath=path.join(root,'builder','working','feature-brain-state.json');
...
function writeState(status,cycles,successes,failures,noProgress){
  ...
  fs.writeFileSync(statePath,JSON.stringify({schemaVersion:7,status,engine:'structured-search-replace-v3',model:process.env.LOCAL_AI_MODEL||'qwen2.5-coder:7b',requestedMinutes,finishGraceMinutes:grace,featureSliceMinutes,maxCycles,maxNoProgress,cycles,successes,failures,consecutiveNoProgress:noProgress,startedAt:new Date(started).toISOString(),normalDeadline:new Date(normalDeadline).toISOString(),hardDeadline:new Date(hardDeadline).toISOString(),elapsedMinutes:Number(((Date.now()-started)/60000).toFixed(2)),remainingMinutes:Number((remaining()/60000).toFixed(2)),updatedAt:new Date().toISOString()},null,2)+'\n');
}
function productSnapshot(){try{return spawnSync('git',['diff','--name-only','--','src','supabase','package.json'],{cwd:root,encoding:'utf8'}).stdout.trim()}catch{return''}}
function runFeature(slice){
  const env={...process.env,
    BUILDER_MAX_MINUTES:String(Math.max(1,slice)),
    LOCAL_AI_MODEL:process.env.LOCAL_AI_MODEL||'qwen2.5-coder:7b',
    ...
    AUTOBOT_FEATURE_DEADLINE_EPOCH_MS:String(hardDeadline),
    AUTOBOT_FEATURE_NORMAL_DEADLINE_EPOCH_MS:String(normalDeadline)
  };
...
appendAudit('knt-long-run-started',{requestedMinutes,grace,featureSliceMinutes,maxCycles,maxNoProgress,engine:'structured-search-replace-v3',model:process.env.LOCAL_AI_MODEL||'qwen2.5-coder:7b'});
while(normalRemaining()>35000&&remaining()>60000&&cycles<maxCycles){
  ...
  if(changed){successes++;noProgress=0;appendAudit('knt-feature-cycle-verified',{cycle:cycles,successes,changedFiles:after.split(/\r?\n/).filter(Boolean)});}
  else{failures++;noProgress++;appendAudit('knt-feature-cycle-no-progress',{cycle:cycles,status:result.status??1,noProgress,maxNoProgress});}
  ...
  if(result.error&&remaining()<60000)break;
  if(remaining()>60000)spawnSync('sleep',['2'],{timeout:5000});
}
const finished={cycles,successes,failures,consecutiveNoProgress:noProgress,elapsedMinutes:Number(((Date.now()-started)/60000).toFixed(2)),remainingMinutes:Number((remaining()/60000).toFixed(2)),engine:'structured-search-replace-v3',model:process.env.LOCAL_AI_MODEL||'qwen2.5-coder:7b'};
writeState('finished',cycles,successes,failures,noProgress);
appendAudit('knt-long-run-finished',finished);
if(!verifyAuditLog().valid)process.exit(3);
console.log(`[autobot] KNT sustained run finished: cycles=${cycles}, verified product passes=${successes}, no-progress=${failures}, elapsed=${finished.elapsedMinutes}m`);
